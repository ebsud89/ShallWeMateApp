...
{
    NSLog(@"Open Menu View Controller");
}
...
- (void)openChildView:(UIView *)childView left:(BOOL)left animated:(BOOL)animated completion:(void (^)(void))completionBlock
{
    ...
    if ([_centerViewController conformsToProtocol:@protocol(YSMenuProtocol)] && [_centerViewController respondsToSelector:@selector(viewWillReduceFromLeft:)])
    ...
    void (^finalBlock)(BOOL) = ^(BOOL finished) {
        ...
        // Warn that view controller will grow
        if ([_centerViewController conformsToProtocol:@protocol(YSMenuProtocol)] && [_centerViewController respondsToSelector:@selector(viewDidReduceFromLeft:)])
...
- (void)closeSideViewControllerAnimated:(BOOL)animated completion:(void (^)(void))completionBlock
{
    void (^finalBlock)(BOOL) = ^(BOOL finished) {
        ...
        if ([_centerViewController conformsToProtocol:@protocol(YSMenuProtocol)] && [_centerViewController respondsToSelector:@selector(viewDidGrow)])
    ...
    // Warn that view controller will grow
    if ([_centerViewController conformsToProtocol:@protocol(YSMenuProtocol)] && [_centerViewController respondsToSelector:@selector(viewWillGrow)])
